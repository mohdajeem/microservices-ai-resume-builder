...
// import { User, Briefcase, GraduationCap, Code, CheckCircle, Plus, Trash2, ArrowRight, ArrowLeft } from 'lucide-react';
...
//   // Initialize state with imported data OR default state
//   const [formData, setFormData] = useState(location.state?.prefilledData || defaultState);
...
//     setLoading(true);
//     try {
//       // The API expects "userData" wrapper
...
//       alert("Failed to create profile. Please check your input.");
//     } finally {
//       setLoading(false);
//     }
//   };
...
//     ArrowRight, ArrowLeft, UploadCloud, FileText, Layout, Loader2 
// } from 'lucide-react';
...
//         setFormData(location.state.prefilledData);
//         setCurrentStep(1); // Skip selection, go straight to editing
//     }
//   }, [location.state]);
...
//     try {
//         // const res = await resumeAPI.getDetail('profile'); // Fetch Master Profile
//         // NEW (Correct)
//         const res = await resumeAPI.getProfile();
//         if (res.data?.data) {
//             // Normalize Master Profile Data (Lowercase in DB) to Wizard Data (Uppercase keys)
...
//             setCurrentStep(1);
//         } else {
//             alert("No Master Profile found. Please create one or start from scratch.");
//         }
//     } catch (error) {
//         console.error(error);
//         alert("Failed to load Master Profile.");
//     } finally {
...
//             <Button 
//                 variant="ghost" 
//                 onClick={() => setCurrentStep(prev => prev === 1 ? 0 : Math.max(1, prev - 1))} // If on Step 1, go back to Step 0 (Selection)
...
//   // ✅ NEW HANDLERS FOR SIMPLE LISTS (Certs/Achievements)
//   // Assuming Backend expects objects {name: "...", link: "..."}
...
//     setLoading(true);
//     try {
//       // Map back to what backend expects (lowercase keys usually)
//       // Note: resumeAPI.create expects { userData: ... }
//       // We send the state as is, assuming backend handles the mapping or keys match schema
...
//       const res = await resumeAPI.create({ userData: payloadData });
